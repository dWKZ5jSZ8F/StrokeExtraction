function skel=kmm(image);
	deletionArray=[3	5	7	12	13	14	15	20	...
				   ...
				   127	131	133	135	141	142	149	151	...
	...
	while change
		outputImage=image>0;
		workingImage=padarray(image,[1 1], 0);
		...
		% this means that it has bits correspoding to any numbers: 2, 8, 32, 128, on and to all of: 1, 4, 16, 64 off.
		on=sum([2	8	32	128]);
		off=sum([1	4	16	64]);
		image=image+((image==2) & bitand(workingImage, on) & ~bitand(workImage, off));
